...
export class TooltipDirective implements OnInit, OnDestroy, OnChanges {
  ...
  // This function to hide tooltip when user is mobile.
  @HostListener('window:touchstart') onblurTouch() {
    if (this.triggers.toString().includes('touchstart') && this.isOpen) {
      this.hide();
    }
  }

  // This function to hide tooltip when user is mobile.
  @HostListener('window:touchmove') onblurTouchMove() {
    if (this.triggers.toString().includes('touchstart') && this.isOpen) {
      this.hide();
    }
  }
